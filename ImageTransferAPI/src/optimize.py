...
def optimize(content_targets, style_target, content_weight, style_weight,
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
            ...
            while iterations * batch_size < num_examples:
                ...
                if should_print:
                    ...
                    if slow:
                       ...
                       #     save_path+"/saved_model"+str(epoch))
                       # signature = predict_signature_def(inputs={'myInput': tf.placeholder(tf.float32)}, outputs={'myOutput': tf.placeholder(tf.float32)})
        ...
        #     inputs={"in": tf.placeholder(tf.float32)},
        #     outputs={
        #         "out": tf.get_default_graph().get_tensor_by_name("add_37:0")},
        #     legacy_init_op=None
        # )
